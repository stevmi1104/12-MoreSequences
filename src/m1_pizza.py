...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(115.0, 60.0),  # All numbers are approximate.
                rg.Point(122.5, 81.7),
                rg.Point(97.5, 81.7),
                rg.Point(85.0, 60.0),
                rg.Point(87.5, 38.3),
                rg.Point(122.5, 38.3)]
    circle = rg.Circle(rg.Point(110, 60), 25)
    answer = generate_points_on_circle(circle, 6)
...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  4 black dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 170)
    draw_points_on_circle(window, circle, 4, 'black')
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    # Tests 1 and 2 (on the same window):
    title = ('POLYGON test 3:  4 segments with thin green lines;')
    window = rg.RoseWindow(200, 200, title)

    circle = rg.Circle(rg.Point(100, 100), 70)
    circle.outline_thickness = 2
...
def run_test_fancy_polygon():
 ...
 # Test 4 (on another window):
    title = ('FANCY POLYGON test 4:  12 purple lines on black circle,'
             + ' hops = 5.')
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 125)
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 12, 5, 'purple', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    circle.attach_to(window)
    for k in range(len(pts)):
        line = rg.Line(pts[k - hops_to_next_point], pts[k])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
